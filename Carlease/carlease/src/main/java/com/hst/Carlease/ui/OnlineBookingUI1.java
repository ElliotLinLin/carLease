...
public class OnlineBookingUI1 extends ThirdBaseUi implements OnClickListener {
    private final String TAG = OnlineBookingUI1.class.getSimpleName();
    ...
    private EditText name, cardno, mine_phone, family, contanct, fa_phone,// 名字，身份证号,亲属名字，亲属关系,亲属电话
            ...
            et_sub_bank_name, now_detail_loc, cardloc //支行名称 详细居住地址
            ;
    ...
    private LinearLayout ll_bar, ll_1, ll_2;

    @Override
    protected void onCreate(Bundle arg0) {
        requestWindowFeature(Window.FEATURE_NO_TITLE);
        super.setContentView(R.layout.ui_onlinebooking1);
                    ...
                    new Thread(new Runnable() {
                        ...
                        public void run() {
                            ...
                            for (AreaBean.ModelBean bean : provinceList) {
                                ...
                                if (tempcity.size() == 0)
                                    tempcity.add(new AreaBean.ModelBean(-1, ""));
...
//                showToast("添加完成");
    ...
    private void getBankList() {
        sendRequest(Http_Url.GETPURCHASEDATALIST, new RequestParams(), PurchaseDataBean.class, new BaseCallBack<PurchaseDataBean>() {
            @Override
            public void onSuccess(final PurchaseDataBean purchaseDataBean) {
    ...
    protected void initControl() {
        ...
        cardloc = (EditText) findViewById(R.id.cardloc);
...
//        message = (TextView) findViewById(R.id.message);
        ...
        cityPicker = new CityPicker.Builder(OnlineBookingUI1.this).textSize(20)
...
//        message.setOnClickListener(this);
        ...
        for (int i = 0; i < ll_bar.getChildCount(); i++) {
            TextView child = (TextView) ll_bar.getChildAt(i);
            child.setTag(i);
            child.setOnClickListener(new OnClickListener() {
                ...
                public void onClick(View v) {
                    ...
                    if (((Integer) v.getTag()) == 0) {
                        child = (TextView) ll_bar.getChildAt(1);
                        ll_1.setVisibility(View.VISIBLE);
                        ll_2.setVisibility(View.GONE);
                    } else {
                        ll_1.setVisibility(View.GONE);
                        ll_2.setVisibility(View.VISIBLE);
                        child = (TextView) ll_bar.getChildAt(0);
                    }
                    child.setBackgroundResource(R.drawable.green_button);
...
//            purchaseParamsBean.setLiveAreaID(intent.getIntExtra("liveAreaID", 0));
            ...
            if (!TextUtils.isEmpty(intent.getStringExtra("cardloc"))) {
                cardloc.setText(intent.getStringExtra("cardloc"));
            }
...
//            }
    ...
    private void CheckPram() {
        ...
        if (cardno.getText().toString().contains(" ")) {
            ToastL.show("身份证号码不能包含空格");
            return;
        }
        if (!StringUtils.isCard(cardno.getText().toString().trim())) {
            ToastL.show("您输入的身份证号码有误");
            return;
        }
        if (cardloc.getText().toString().trim().isEmpty()) {
            ToastL.show("请输入您的身份证地址");
            return;
        }
...
//        intent.putExtra("CompanyID", id);//业务网点
        ...
        intent.putExtra("cardloc", cardloc.getText().toString().trim());
