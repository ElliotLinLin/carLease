...
public abstract class AbsDialogFgm extends android.support.v4.app.DialogFragment {

	private static final String TAG = AbsDialogFgm.class.getSimpleName();
	...
	// 加载器Id
	protected int loaderId = 0; // 不知道会不会跟UI里的loader冲突？
	// 加载器回调
	protected LoaderCallbacks<byte[]> loaderCallbacks = null;
	// 任务加载器
	protected AbsTaskLoaderHttpWait<byte[]> taskLoader = null;

	// 等待控件
	protected WaitFgm waitFgm = null;
	protected int waitViewId = 0;

	protected LoaderConfig loaderConfig = new LoaderConfig();
	...
	private void addWaitFgm() {
		...
		if (waitFgm == null) {
			...
			fragmentManager.addOnBackStackChangedListener(new android.support.v4.app.FragmentManager.OnBackStackChangedListener() {
				...
				public void onBackStackChanged() {
					Log.e(TAG, "addWaitFgm()---addOnBackStackChangedListener:onBackStackChanged()");
				}
		...
		if (waitFgm.isAdded() == false) {
			...
			Log.e(TAG, "addWaitFgm()---11111111111111111()");
	...
	private void showWaitFgm() {
		...
		if (waitFgm.isVisible() == false) { // 如果WaitFgm不显示
			// 显示
			if (waitFgm.getView() == null) {
				Log.e(TAG, "waitFgm.getView() == null");
			}else{
				Log.e(TAG, "waitFgm.getView() != null");
			}
...
//	}
	...
	public void onViewCreated(View view, Bundle savedInstanceState) {
		getDialog().setCanceledOnTouchOutside(true); // 设置点击外部关闭
