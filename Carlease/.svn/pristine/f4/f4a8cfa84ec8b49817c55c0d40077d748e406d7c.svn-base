...
public class GJson {
	...
	public static <T> T trimObjectString(T entity, Class<T> clazz) {
		...
		for(int i = 0 ; i < len; i++) {
			...
			if (D) Log.i(TAG, "field name:"+field.getName());

			// 判断：是String类型，并且没有final修饰符
			if ( field.getType() == java.lang.String.class ) {
				if (D) Log.i(TAG, "== String.class");
				if ( java.lang.reflect.Modifier.isFinal(field.getModifiers()) ) {
					if (D) Log.i(TAG, "== Final");
					continue;
				} else {
					if (D) Log.i(TAG, "!= Final");
					try {
	 ...
	 * @return
	 */
	public static <T> List<T> parseList(String jsonObjectString, String keyName, Class<T> clszz, Mode mode) {
	 ...
	 * @return
	 */
	public static <T> List<T> parseList(String jsonObjectString, String keyName, Class<T> clszz) {
		return parseList(jsonObjectString, keyName, clszz, Mode.NOT);
	}
	 ...
	 * @return
	 */
	public static <T> List<T> parseList(String jsonArrayString, Class<T> clszz, Mode mode) {
	 ...
	 * @return
	 */
	public static <T> List<T> parseList(String jsonArrayString, Class<T> clszz) {
		return parseList(jsonArrayString, clszz, Mode.NOT);
	}
