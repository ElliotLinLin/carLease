...
public class AbWheelView extends View {
	...
	/** Items text color. */
	private static final int ITEMS_TEXT_COLOR = 0xFFEAEAEA;

	/** Top and bottom shadows colors. */
	// private static int[] SHADOWS_COLORS = new int[] { 0xFF111111, 0x00AAAAAA,
	// 0x00AAAAAA };
	private static int[] SHADOWS_COLORS = new int[] { 0x00FFFFFF, 0x00FFFFFF,
			0x00FFFFFF };
	/** Label offset. */
	...
	private int[] centerSelectGradientColors = new int[] { 0x00FFFFFF,
	...
	private int centerSelectStrokeColor = 0x00FFFFFF;
	...
	/** Current value. */
	private int valueTextColor = 0xFF000000;
	...
	private void initResourcesIfNecessary() {
		...
		if (valuePaint == null) {
			...
			// valuePaint.setTextSize(valueTextSize);//TODO 选中的字体大小
			valuePaint.setTextSize(valueTextSize + 2);
		}
	...
	private int calculateLayoutWidth(int widthSize, int mode) {
		...
		if (maxLength > 0) {
			// 一个字符宽度
			float textWidth = (int) AbAppUtil.getStringWidth("0", labelPaint);
			// 不算lable的宽度
		...
		if (label != null && label.length() > 0) {
			labelWidth = (int) AbAppUtil.getStringWidth(label, labelPaint);
		}
