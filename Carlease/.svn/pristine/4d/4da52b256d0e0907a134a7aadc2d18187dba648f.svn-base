...
public class StatusLogFileHandler extends StreamHandler {
     ...
     */
    public StatusLogFileHandler(String fileUrl, int limit, int dateInter, boolean append) throws Exception {
        super();
     ...
     * @throws Exception
     */
    public StatusLogFileHandler(String fileUrl, boolean append) throws Exception {
        super();
        this.fileUrl = fileUrl;
        this.append = append;
        openWriteFiles();
    ...
    private void getLastFile() {
        try {
            ...
            Logger.getLogger(StatusLogFileHandler.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
        ...
        try {
            ...
            while ((line = br.readLine()) != null) {
                if (line.trim().length() != 0) {
                    if (file.isDirectory()) { //是个目录
//                    	Log.e(TAG, "StatusLogFileHandler check logFiles get  "+
//                    			fileUrl + File.separator + "未命名" + "_" + line + ".log");
...
//                Log.e(TAG,"StatusLogFileHandler checkLogFile day "
//                +(nowDate - sdf.parse(file.toString(), new ParsePosition(file.toString().lastIndexOf('_') + 1)).getTime()) / (86400 * 1000));
                //筛选过期的文件(3天过期)
                if ((nowDate - sdf.parse(file.toString(), new ParsePosition(file.toString().lastIndexOf('_') + 1)).getTime()) / (86400 * 1000) > dateInter) {
                    overdueLog.add(file);
